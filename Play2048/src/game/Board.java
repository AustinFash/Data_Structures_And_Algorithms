...
public class Board {
    ...
    public void updateOpenSpaces() {
        ...
        for (int r = 0; r < board.length; r++ ) {
            for (int c = 0; c < board[r].length; c++ ) {
                if (gameBoard[r][c] == 0){
                    BoardSpot newSpot = new BoardSpot(r, c);
                    openSpaces.add(newSpot); 
                }
            }
        }   
    }
    ...
    public void addRandomTile() {
        // WRITE YOUR CODE HERE
       int num = StdRandom.uniform(0,openSpaces.size());
       double num2 = StdRandom.uniform(0.0,1.0);
       int row = (openSpaces.get(num)).getRow();
       int col = (openSpaces.get(num)).getCol();
       if(num2 >= 0.1){
            gameBoard[row][col] = 2;
       } else if(num2 < .1){
            gameBoard[row][col] = 4;
        ...
        for(int r = 0; r < gameBoard.length; r++){
            for(int c = 1; c < gameBoard[r].length; c++){
                if(gameBoard[r][c] != 0){
                    ...
                    num = 1;
                    while(check == true){
                        if(gameBoard[r][c-num] == 0){
                            gameBoard[r][c-num] = gameBoard[r][c-num+1];
                            gameBoard[r][c-num+1] = 0;
                            num++;
                        }
                        if(c - num < 0){
                            check = false;
                        }else if(gameBoard[r][c-num] != 0){
                            check = false;
                        }
                    }
                }
    ...
    public void transpose() {
        ...
        for(int r = 0; r < gameBoard.length; r++){
            for(int c = 0; c < gameBoard[r].length; c++){
                temp[c][r] = gameBoard[r][c];
            }
         }
         for(int r = 0; r < gameBoard.length; r++){
            for(int c = 0; c < gameBoard[r].length; c++){
                gameBoard[c][r] = temp[c][r];
            }
    ...
    public void flipRows() {
        ...
        for(int r = 0; r < gameBoard.length; r++){
            numCol = 3;
            for(int c = 0; c < gameBoard[r].length; c++){
                temp[r][c] = gameBoard[r][numCol];
                numCol--;
            }
         }
         for(int r = 0; r < gameBoard.length; r++){
            for(int c = 0; c < gameBoard[r].length; c++){
                gameBoard[r][c] = temp[r][c];
            }
